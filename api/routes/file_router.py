...
async def file_upload(
    file: Dict[str, List[Dict[str, Any]]],  # Expecting a dict with a "file" key holding the list
    db: Session = Depends(get_db),
    current_user: dict = Depends(get_current_user)
):
    # Access the file list
    object_data = file["file"]  # 'file' is a dict, so we access the list via file["file"]
    ...
    for i in range(0, len(object_data)):
        processed_data = processData(object_data[i])  # Assuming each dict has the necessary data
        current_values = {
