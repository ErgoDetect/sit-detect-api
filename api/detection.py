class detection:
    def __init__(self, frame_per_second=1, correct_frame=5):
        ...
        self.result = [False, False, False, False]  # [blink_alert, sitting_alert, distance_alert, thoracic_alert]
    ...
    def set_correct_value(self,input):
        ...
        if (self.response_counter == self.correct_frame): 
            self.correct_values = {
                "shoulderPosition": average([v['shoulderPosition'] for v in self.saved_values]),
                "diameterRight": average([v['diameterRight'] for v in self.saved_values]),
    ...
    def detect(self,input,faceDetect):
        ...
        if (self.response_counter>self.correct_frame):
            if input["shoulderPosition"] == None:
                self.thoracic_stack = 0
            elif self.correct_values["shoulderPosition"]+0.05<=input["shoulderPosition"]:
                self.thoracic_stack += 1
            else : self.thoracic_stack = 0 
            ...
            if faceDetect == False:
                ...
                (ear_right is not None and ear_right <= self.ear_threshold_low)):
                    ...
                    (ear_left is not None and ear_left >= self.ear_threshold_high) or
                    (ear_right is not None and ear_right >= self.ear_threshold_high)):
            ...
            if(self.blink_stack >= self.blink_stack_threshold * self.frame_per_second ):
                if(self.result[0]==False):
                    ...
                    self.timeline_result["blink"][len(self.timeline_result["blink"])-1].append(self.response_counter-self.blink_stack_threshold)
                self.result[0] = True
            else:
                if(self.result[0]==True):
                    self.timeline_result["blink"][len(self.timeline_result["blink"])-1].append(self.response_counter)
                self.result[0] = False

            if(self.sitting_stack >= self.sitting_stack_threshold * self.frame_per_second ):
                if(self.result[1]==False):
                    self.timeline_result["sitting"].append([]) 
                    self.timeline_result["sitting"][len(self.timeline_result["sitting"])-1].append(self.response_counter-self.sitting_stack_threshold)
                self.result[1] = True
            else:
                if(self.result[1]==True):
                    self.timeline_result["sitting"][len(self.timeline_result["sitting"])-1].append(self.response_counter)
                self.result[1] = False

            if(self.distance_stack >= self.distance_stack_threshold * self.frame_per_second ):
                if(self.result[2]==False):
                    self.timeline_result["distance"].append([]) 
                    self.timeline_result["distance"][len(self.timeline_result["distance"])-1].append(self.response_counter-self.distance_stack_threshold)
                self.result[2] = True
            else:
                if(self.result[2]==True):
                    self.timeline_result["distance"][len(self.timeline_result["distance"])-1].append(self.response_counter)
                self.result[2] = False

            if(self.thoracic_stack >= self.thoracic_stack_threshold * self.frame_per_second ):
                if(self.result[3]==False):
                    self.timeline_result["thoracic"].append([]) 
                    self.timeline_result["thoracic"][len(self.timeline_result["thoracic"])-1].append(self.response_counter-self.thoracic_stack_threshold)
                self.result[3] = True
            else:
                if(self.result[3]==True):
                    self.timeline_result["thoracic"][len(self.timeline_result["thoracic"])-1].append(self.response_counter)
                self.result[3] = False
