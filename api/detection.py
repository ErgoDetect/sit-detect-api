...
#             False,
#             False,
#         ]  # [blink, sitting, distance, thoracic]
...
#     def detect(self, input, face_detect):
#         self.response_counter += 1
#         if self.response_counter > self.correct_frame:
#             self.update_thoracic(input)
#             if face_detect:
#                 self.update_sitting()
#                 self.update_distance(input)
#                 self.update_blink(input)
#             else:
#                 self.reset_stacks()

#             self.update_result("blink", self.blink_stack, self.blink_stack_threshold)
#             self.update_result(
#                 "sitting", self.sitting_stack, self.sitting_stack_threshold
#             )
#             self.update_result(
#                 "distance", self.distance_stack, self.distance_stack_threshold
#             )
#             self.update_result(
#                 "thoracic", self.thoracic_stack, self.thoracic_stack_threshold
#             )

#     def update_thoracic(self, input):
#         if input["shoulderPosition"] is None:
#             self.thoracic_stack = 0
#         elif (
#             self.correct_values["shoulderPosition"] + 0.05 <= input["shoulderPosition"]
#         ):
#             self.thoracic_stack += 1
#         else:
#             self.thoracic_stack = 0
...
#     def update_distance(self, input):
#         diameter_right = input.get("diameterRight", self.latest_nearest_distance)
#         diameter_left = input.get("diameterLeft", self.latest_nearest_distance)
#         self.latest_nearest_distance = max(diameter_right, diameter_left)

#         correct_diameter_right = self.correct_values.get("diameterRight", 0)
#         correct_diameter_left = self.correct_values.get("diameterLeft", 0)
#         correct_distance = max(correct_diameter_right, correct_diameter_left)

#         if correct_distance * 1.10 <= self.latest_nearest_distance:
#             self.distance_stack += 1
#         else:
#             self.distance_stack = 0

#     def update_blink(self, input):
#         ear_left = input.get("eyeAspectRatioLeft")
#         ear_right = input.get("eyeAspectRatioRight")
...
#     def update_result(self, key, stack, threshold):
#         if stack >= threshold * self.frame_per_second:
#             if not self.result[self.get_index(key)]:
#                 self.timeline_result[key].append([self.response_counter - threshold])
#             self.result[self.get_index(key)] = True
#         else:
#             if self.result[self.get_index(key)]:
#                 self.timeline_result[key][-1].append(self.response_counter)
#             self.result[self.get_index(key)] = False

#     def get_index(self, key):
#         return ["blink", "sitting", "distance", "thoracic"].index(key)
